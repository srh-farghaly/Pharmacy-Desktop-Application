...
public class customers_operations {
   ...
   {
       String query="insert into customer (firstname,lastname,city,street,gender,phoneNumber_1) values ('"+firstname+"' , '"+lastname+"' , '"+city+"' , '"+street+"' , '"+gender+"', '"+phoneNumber_1+"')";
      ...
      public  static ArrayList<Customers_Model> get_customersData()
    {
        ...
        try {
            while(rs.next())
            {
                arr.add(new Customers_Model(rs.getInt("customer_id"),rs.getString("firstname"),rs.getString("lastname"),rs.getString("city"),rs.getString("street"),rs.getString("gender"),rs.getString("phoneNumber_1")));
            }
            return arr;
    ...
    {   
        ...
        try {
            while(rs.next())
            {
              obj= new Customers_Model(rs.getInt("customer_id"),rs.getString("firstname"),rs.getString("lastname"),rs.getString("city"),rs.getString("street"),rs.getString("gender"),rs.getString("phoneNumber_1"));  
            }       
        } catch (SQLException ex) {
          JOptionPane.showMessageDialog(null, ex, "Message", JOptionPane.ERROR_MESSAGE);
         obj=null;
